  ...
  return (
    ...
    >
      <div className="px-3 py-2.5" ref={measureRef}>
        {title && (
          <div className="mb-2 font-medium text-[var(--chart-tooltip-foreground)] text-xs">
            {title}
          </div>
        )}
          ...
          {rows.map((row) => (
            ...
            >
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-[var(--chart-tooltip-muted)] text-sm">
                  {row.label}
                </span>
              </div>
              <span className="font-medium text-[var(--chart-tooltip-foreground)] text-sm tabular-nums">
  ...
  return createPortal(
    ...
    >
      <motion.div
        ...
        className="min-w-[140px] overflow-hidden rounded-lg bg-[var(--card-bg)] text-[var(--text-primary)] shadow-lg backdrop-blur-md border border-[var(--border-color)]"
...
function XAxisLabel({
  ...
  return (
    ...
    >
      <motion.span
        animate={{ opacity }}
        className={cn("whitespace-nowrap text-[var(--chart-label)] text-xs")}
        initial={{ opacity: 1 }}
  ...
  return createPortal(
    <div className="pointer-events-none absolute inset-0">
      {ticks.map((tick) => (
        ...
        >
          <span className="whitespace-nowrap text-[var(--chart-label)] text-xs tabular-nums">
            {tick.label}
          </span>
        </div>
      ))}
